...
const UpLoadDropZone = ({ setData, file, setFile }) => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    const result = {
      orderNumber: `OD00${formData.get("orderNumber")}`,
      orderDate: formData.get("orderDate"),
      dueDate: formData.get("installationDeadline"),
      type: {
        name: selected.name,
        typeID: `נ${formData.get("orderNumber")}`,
      },
      client: {
    ...
    try {
      const response = await fetch("http://localhost:9001/api/v1/cards", {
                  ...
                  <form onSubmit={handleFormSubmit}>
                    <div className="<div grid grid-cols-1 mb-28 sm:grid-cols-2 md:grid-cols-3 gap-6 text-[#040415]></div>">
                      ...
                      <div className="form-control w-full max-w-xs">
                        ...
                        <div className="flex justify-center relative font-medium input border-2 border-none shadow-sm p-0 input-bordered w-full text-right rounded-md">
                          <div
                            onClick={toggleDropdown2}
                            className={`flex items-center gap-2 px-3 border-2 ${
                              isOpen2
                                ? "border-[#00A5FF] text-[#00A5FF]"
                                : "border-[#ebeaed] text-black"
                            } bg-white cursor-pointer hover:bg-gray-50 shadow-sm rounded-md w-full text-right`}
                          >
                            <span className="w-7 h-7 bg-[#00a6ff27] p-[6px] rounded-full flex items-center justify-center text-[#00A5FF]">
                              {selected.icon()}
                            </span>{" "}
                            {selected.name}
                            <svg
                          ...
                          {isOpen2 && (
                            <ul className="absolute top-10 z-10 w-full bg-white border-[#ebeaed] border-2 shadow-lg rounded-md mt-3 px-3 py-4 flex flex-col gap-3">
                              {types.map((item) => (
                                ...
                                >
                                  <span
                                    className={`w-7 h-7 p-[6px] rounded-full flex items-center justify-center group-hover:bg-[#00a6ff27] ${
                                      item.id === selected.id
                                        ? "bg-[#00a6ff27]"
                                        : "bg-[#EAE9EC]"
                                    }`}
